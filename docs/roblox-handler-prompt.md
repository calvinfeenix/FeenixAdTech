...
- **Per-client** detection (camera is per-player) on a throttled Heartbeat/RenderStepped loop, using the **5-gate + dwell** algo from `ImpressionAlgo.luau`:
  1. Proximity (≤ max track distance) 2. Viewport (`Camera:WorldToViewportPoint` OnScreen) 3. **Face orientation** — count ONLY when the viewer is looking at the actual face the ad is displayed on: use the SurfaceGui's `Face` to compute the world outward normal and require `faceNormal·(camera→faceCenter) ≥ threshold` (so viewing the back/side never counts) 4. Occlusion (multi-point raycast across the face surface; **exclude ALL players' characters** so avatars never block an ad) 5. **Dwell** — all gates continuously true for `IMPRESSION_DWELL_SECONDS` (**default 1.0**, configurable). **Hard reset on any gate failure.**
