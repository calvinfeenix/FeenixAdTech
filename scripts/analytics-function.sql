-- ════════════════════════════════════════════════════════════════════════
-- Feenix AdTech — analytics aggregation RPC
-- Paste this whole block into the Supabase SQL editor and Run.
--
-- Why: the dashboard/campaign pages used to fetch RAW analytics events and
-- aggregate in JS. PostgREST caps a plain select at 1000 rows and returns the
-- OLDEST ones, so once events passed ~1000 the charts silently stopped at the
-- earliest days (e.g. the 15th) even though newer data exists. This function
-- aggregates entirely in SQL and returns one small JSON payload, so it never
-- hits that cap. SECURITY INVOKER → the caller's RLS still applies.
-- ════════════════════════════════════════════════════════════════════════
